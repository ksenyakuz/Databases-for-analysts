...
?
-- Что делаем:
-- 1. Определяем критерии для каждой буквы R, F, M (т.е. к примеру, R – 3 для клиентов, которые покупали <= 30 дней от последней даты в базе, R – 2 для клиентов, 
-- которые покупали > 30 и менее 60 дней от последней даты в базе и т.д.)
...
-- 5. Проверяем, что общее кол-во пользователей бьется с суммой кол-во пользователей по группам из п. 3 (если у вас есть логические ошибки в создании групп, 
...
7 и 8 столбцы - оконная функция, поэтому значения для одинаковых статусов проставлены одинаковые, в сумме 3 таких статуса дают 100%
получившаяся таблица в Excel
 */
...
-- Проверим, что общее кол-во пользователей бьётся с суммой кол-ва пользователей по группам из п. 3:

SELECT 
	SUM(s.`count`) AS `пользователей в RFM`,
	@total_users_in_the_database AS `всего пользователей в базе`,
	(CASE WHEN (SELECT SUM(s.`count`) = (SELECT COUNT(DISTINCT user_id) FROM new_table_orders_4)) = 1 THEN 'да' ELSE 'нет'END) AS `совпадает ли кол-во?`
FROM
