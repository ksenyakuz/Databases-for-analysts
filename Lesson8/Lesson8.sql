...
BEGIN
	...
	-- lost - с 1 заказом больше 90 дней назад.
	...
	-- проверим количество строк и общую сумму у получившейся таблицы, они должны совпадать с уникальным количеством в new_table_orders
	SELECT 
	...
	-- заполняем данными итоговую таблицу new_table_orders_8
...
-- и так в цикле прибавляем по месяцу и получаем на конец каждого всё новые и новые данные в таблицу new_table_orders_8
...
/*
- regular - эта категория оказалась одной из самых стабильных и за 6 месяцев практически не изменилась. Она пополняется новыми, которые через 
...
- new - категория к началу июля снизилась на 6,62%, это связано с общим небольшим снижением кол-ва заказов (в особенности ближе к лету). Стоит обратить внимание на неё, 
так как в марте было самое ощутимое падение новых, несмотря на 8-е число (праздничный день)
- при этом количество крупных заказов растёт, так как выросли категории vip и supervip на 12,4 и 5,85 % соответственно
это согласуется и с общим ростом магазина, который хорошо видно в ежемесячной разбивке данных по сумме заказов
...
туда отправляются. Именно поэтому так важно стимулировать рост новых покупателей
*/
